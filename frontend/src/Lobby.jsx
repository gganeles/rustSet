...
export default function Lobby(props) {
  ...
  async function createGame(e) {
    ...
    socket.send(
      JSON.stringify({ kind: "create_game", data: JSON.stringify(payload) }),
    );
    setGameError("");
  }
  ...
  function handleSaveName(val) {
    ...
    if (!v) return;
    try {
      localStorage.setItem("rs_name", v);
    } catch (e) {}
    setCreator(v);
    setShowNameModal(false);
    setNameError("");
  ...
  function joinGame(id) {
    ...
    if (socket && socket.readyState === WebSocket.OPEN) {
      ...
      socket.send(
        JSON.stringify({ kind: "join_game", data: JSON.stringify(payload) }),
      );
    }
  ...
  function deleteGame(id) {
    ...
    const payload = { id };
    socket.send(
      JSON.stringify({ kind: "delete_game", data: JSON.stringify(payload) }),
    );
  }
    ...
    <div class="min-h-screen bg-gradient-to-br from-indigo-50 via-purple-50 to-pink-50">
      ...
      <div class="max-w-5xl mx-auto p-3 sm:p-6">
        ...
        <div class="text-center mb-4 sm:mb-8">
          <h1 class="text-3xl sm:text-4xl font-bold text-indigo-900 mb-1 sm:mb-2">
            Game Lobby
          </h1>
        ...
        <div class="bg-white rounded-xl sm:rounded-2xl shadow-lg p-4 sm:p-6 mb-3 sm:mb-6 border border-purple-100">
          ...
          <form onSubmit={createGame} class="flex flex-col gap-3 sm:gap-4">
            <div class="flex flex-col w-full sm:flex-row gap-2 sm:gap-3">
              <div class="w-full flex-1">
                <select
                  class="w-full flex-2 px-3 py-2 sm:px-4 sm:py-3 border-2 border-gray-200 rounded-lg sm:rounded-xl focus:outline-none focus:border-purple-500 focus:ring-2 focus:ring-purple-200 transition-all bg-white cursor-pointer text-sm sm:text-base"
                  value={gameType()}
          ...
          <div class="space-y-2 sm:space-y-3">
            {games().length === 0 ? (
              ...
              games().map((g, i) => (
                <div
                  key={i}
                  class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2 sm:gap-3 p-3 sm:p-4 border-2 border-gray-100 rounded-lg sm:rounded-xl hover:border-pink-200 hover:bg-pink-50/50 transition-all"
                >
                  <div class="flex-1">
                    <div class="font-semibold text-gray-800 text-base sm:text-lg">
                      {g.name}
                    </div>
                    <div class="text-xs sm:text-sm text-gray-600 mt-0.5 sm:mt-1 flex flex-wrap items-center gap-x-3 gap-y-1">
                      ...
                      <span class="flex items-center gap-1">
                        ...
                        {g.players_online}{" "}
                        {g.players_online === 1 ? "player" : "players"}
                      </span>
                    </div>
                  </div>
